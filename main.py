...
async def train_all_parameters(collection_name: str, window_size: int = 10, epochs: int = 50):
    """Entrenar modelos para todos los parámetros (temperatura, pH, oxígeno)"""
    try:
        results = {}
        parameters = ['temperatura', 'ph', 'oxigeno']
        ...
        for param in parameters:
            try:
                logger.info(f"Entrenando modelo para {param}")
                
                # Obtener datos específicos del parámetro
                data = await mongo_client.get_time_series_data(
                    collection_name=collection_name,
                    parameter=param
                )
                ...
                if data:
                    # Crear modelo específico para este parámetro
                    param_model = PerceptronTimeSeries()
                    metrics = param_model.train(
                        data=data,
                    ...
                    results[param] = {
                        "status": "no_data",
                        "message": f"No se encontraron datos para {param}"
                    }
        ...
        logger.error(f"Error entrenando todos los parámetros: {e}")
        raise HTTPException(status_code=500, detail=str(e))
